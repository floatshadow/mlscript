...
class Function private (val name: String, val args:  List[Operand.Var], val isMain: Boolean, val locals: Map[String,(Type,Boolean)], val code: Code) {
...
object Function {
  def apply(name: String, args: List[Operand.Var], isMain: Boolean)(codeGen: LocalsHandler => Code) = {
    ...
    val code = codeGen(lh)
    new Function(name, args, isMain, lh.getLocals.toMap, code)
  }
}
